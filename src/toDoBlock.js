...
function sortForms(allTodoForms, sortedTodos) {
  return [...allTodoForms].sort((a, b) => {
    ...
    const correspondingTodoA = sortedTodos.map(
      (todoContainer) => todoContainer.dataset.todoNum === dataIDA,
    );
    const correspondingTodoB = sortedTodos.map(
...
function appendSortedTodos(sortedForms, sortedTodos) {
  const main = document.querySelector('main');
  for (let i = 0; i < sortedForms.length; i += 1) {
...
function changeTodoNum(sortedTodos, sortedForms) {
  for (let i = 0; i < sortedTodos.length; i += 1) {
...
function makeTodoBlock(
  ...
  removeButton.selectedTodoContainer = todoContainer;
  checkbox.selectedTodoContainer = todoContainer;
